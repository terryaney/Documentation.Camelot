...
`contentSelector`<sup>1</sup> | `string` | If the content for the modal being displayed is generated by the current application, a DOM element selector string can be passed versus having to build a Kaml View. When `contentSelector` is passed, the KatApp Framework will clone the element's content.
...
<sup>1</sup> When the `contentSelector` property is provided, but you still want to be able to use Vue directives (especially reactivity and events) in the modal dialog, you can decorate the element matched by `contentSelector` with the Vue directive of `v-pre`.  This results in none of the Vue/KatApp directives processing until the modal is actually displayed.  When the modal starts, it will receive a clone of the host applications `rbl`, 'model', and 'handlers' objects. 
