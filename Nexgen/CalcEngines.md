...
{xDS.groupName} | If the endpoint is actually an api call to the KAT xDS API, the route segment for group can be provided via `xDS.groupName` where `groupName` is the name of an `appSettings` value.  For example, an `appSettings.severanceGroup` with a value of `DeltaSeverance` could be used via `{xDS.severanceGroup}` token in an endpoint url and it would be substituted with `DeltaSeverance`.  These values should be specified in the `RBLSite.appSettings` table of client BRDs.
