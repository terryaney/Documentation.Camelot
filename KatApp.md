...
---|---|---
`kaId` | `string` | Current id of the running KatApp.  Typically only used in Template Files when a unique id is required.  If a template is rendered more than one time on a page, use [`$renderId`](#template-script-and-style-tags) instead as it provides a unique id for each template render.
...
- [v-pre](#v-pre) - Use the `v-pre` directive to an element that is used for [IModalOptions.contentSelector](#imodaloptions) if the markup within the element should not be processed by the host application, but instead should be processed and become reactive when the modal application is created.
...
Use the `v-pre` directive to an element that is used for [IModalOptions.contentSelector](#imodaloptions) if the markup within the element should not be processed by the host application, but instead should be processed and become reactive when the modal application is created.
